...
// ---------------------------------------------------------------------------
// Mezcla determinista de opciones: la posición de la respuesta correcta varía
// de un ejercicio a otro (semilla basada en el id), pero se mantiene estable
// entre sesiones para no invalidar el progreso guardado ni causar errores de
// hidratación (mismo resultado en servidor y cliente).
// ---------------------------------------------------------------------------

function seededShuffle<T>(items: readonly T[], seedKey: string): T[] {
  // Semilla FNV-1a a partir de la clave.
  let seed = 2166136261;
...
export default function ExerciseRunner({ practice }: { practice: PracticeUnit }) {
  ...
  // Opciones mezcladas: la letra (A, B, C, D) se asigna según la posición
  // mostrada; la selección sigue usando el id original de la opción.
  const displayOptions = useMemo(
    () => seededShuffle(exercise.options, `${practice.id}:${exercise.id}`),
    [practice.id, exercise]
  );
  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-4 sm:px-6">
      ...
      >
        ...
        {exercise.diagramSvg && <DiagramSvg svg={exercise.diagramSvg} />}
